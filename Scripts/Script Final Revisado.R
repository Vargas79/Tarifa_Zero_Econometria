...
# Inclui:
#   - Exclusão da coorte 2018 (outlier)
#   - Filtro de anos de tratamento válidos (2015–2023)
#   - DiD Clássico, PSM+DiD, Callaway & Sant'Anna (2021)
#   - Testes de robustez: not yet treated, max_e = 4 e 5
# ==============================================================================
...
# ── 2. BASE UNIFICADA, FILTRO DE ANOS E EXCLUSÃO DA COORTE 2018 ─────────────
cat("\n[3/9] Montando painel, filtrando anos e excluindo coorte 2018...\n")
...
# ── FILTRAR ANOS DE TRATAMENTO PLAUSÍVEIS (2015–2023) ──
# Remove municípios com ano de tratamento anterior a 2015 ou posterior a 2023
ano_minimo <- 2015
ano_maximo <- 2023

df_base_raw <- df_base_raw %>%
  mutate(
    ano_invalido = ifelse(!is.na(ano_trat_num) & 
                            (ano_trat_num < ano_minimo | ano_trat_num > ano_maximo), 
                          TRUE, FALSE)
  )

# Remover municípios com ano inválido
municipios_invalidos <- df_base_raw %>%
  filter(ano_invalido == TRUE) %>%
  distinct(municipio_id) %>%
  pull(municipio_id)

cat(sprintf("   Removendo %d municípios com ano de tratamento fora do intervalo %d-%d\n", 
            length(municipios_invalidos), ano_minimo, ano_maximo))

df_base_raw <- df_base_raw %>% filter(!municipio_id %in% municipios_invalidos)

# Identificar e remover a coorte 2018 (outlier)
...
cat(sprintf("   Painel final (sem 2018 e com anos válidos): %d observações | %d municípios\n",
...
cat("\n╔══════════════════════════════════════════════════════════════╗\n")
cat("║     TABELA CORRIGIDA (SEM COORTE 2018 E ANOS VÁLIDOS)      ║\n")
cat("╚══════════════════════════════════════════════════════════════╝\n")
...
# ── 10. CONTAGEM DE MUNICÍPIOS (PARA ATUALIZAR A APRESENTAÇÃO) ────────────
cat("\n[10/9] Contagem de municípios (amostra final corrigida)...\n")

n_total <- n_distinct(df_base$municipio_id)
n_tratados <- df_base %>%
  filter(!is.na(ano_trat_num) & ano_trat_num >= 2015 & ano_trat_num <= 2023) %>%
  distinct(municipio_id) %>%
  nrow()

n_controles <- n_total - n_tratados

distribuicao_coortes <- df_base %>%
  filter(!is.na(ano_trat_num) & ano_trat_num >= 2015 & ano_trat_num <= 2023) %>%
  distinct(municipio_id, ano_trat_num) %>%
  group_by(ano_trat_num) %>%
  summarise(n = n()) %>%
  arrange(ano_trat_num)
...
print(distribuicao_coortes)

# Salvar contagem em CSV
write.csv(distribuicao_coortes, "distribuicao_coortes.csv", row.names = FALSE)
